...
export function useRfmApp(){
  ...
  const downloadMap=useCallback(async()=>{
    ...
    let staged=null;
    const previous=currentPackage.offlineMap||null;
    setMapProgress({text:'Подготавливаю карту…',busy:true});
    try{
      if(navigator.storage?.persist) try{await navigator.storage.persist();}catch{}
      staged=await downloadOfflineMap(currentPackage,p=>setMapProgress({
        text:`Скачано ${p.saved} тайлов · ${formatBytes(p.bytes)}${p.failed?` · ошибок ${p.failed}`:''}`,
        button:`Карта ${p.done}/${p.total}`,busy:true
      }));
      const pkg={...currentPackage,offlineMap:staged};
      await savePackage(pkg);staged=null;
      if(previous) try{await discardOfflineMapRevision(previous,pkg.id);}catch(error){console.warn('Could not remove previous offline map revision',error);}
      setCurrentPackage(pkg);await refreshPackages(pkg.id);
    }catch(error){
      if(staged) try{await discardOfflineMapRevision(staged);}catch{}
  ...
  const downloadTerrainForRace=useCallback(async()=>{
    ...
    let staged=null;const previous=currentPackage.terrain||null;
    setTerrainProgress({text:'Подготавливаю рельеф…',busy:true});
    try{
      staged=await downloadTerrain(currentPackage,p=>setTerrainProgress({
        text:`Скачано ${p.saved} DEM-тайлов · ${formatBytes(p.bytes)}${p.failed?` · ошибок ${p.failed}`:''}`,
        button:`Рельеф ${p.done}/${p.total}`,busy:true
      }));
      const pkg={...currentPackage,terrain:staged};
      await savePackage(pkg);staged=null;
      if(previous?.storageId&&previous.storageId!==pkg.terrain.storageId) try{await discardTerrainRevision(previous);}catch{}
      setCurrentPackage(pkg);await refreshPackages(pkg.id);
    }catch(error){
      if(staged) try{await discardTerrainRevision(staged);}catch{}
