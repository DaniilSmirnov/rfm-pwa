import { API_ORIGIN, BASEMAP_PM, commonHeaders, json } from './shared.js';
...
export async function proxyRallyFans(request, url) {
...
export async function importYandexConstructor(request, url) {
...
export async function proxyBasemap(request) {
...
export async function proxyRfmFont(request,url){
...
export async function proxyRfmIcon(){
  const upstream=await fetch('https://rallyfansmap.ru/assets/icons/apple-touch-icon.png',{cf:{cacheEverything:true,cacheTtl:604800}});
  return new Response(upstream.body,{status:upstream.status,headers:commonHeaders({'content-type':'image/png','cache-control':'public, max-age=604800'})});
}
